...
- checkpoint 69後のcomponent診断では、body-onlyが均一な壁紙、specular-onlyが孤立白点として読まれ、single-pass final sensorはその空間分布を主因として潰していなかった。body帯域、等方roughness、光源径、meso mean-normal、局所roughness統計を操作したcheckpoint 70〜77も、反射像を連結せず平滑化か白粒化へ戻った。次段は固定emitter、macro構図、色階層、fine / coarse band separationを基準に、scene radianceとspecular luminanceを一枚のHDR render targetへ分離する。撮像passはspecular成分だけを小半径PSFでenergy-preservingに再構成し、高輝度域に限る低energy glareを加えてからsensor responseへ通す。diffuse / ambient / height detailは空間filterへ通さず、PSFをdetail生成maskとして使わない。body diffuse帯域とspecular平均方向は操作比較できるが、coarse reliefをbodyへ戻さず、波・ぼけ・白粒を再発させない。非対応GPUはsingle-passへfallbackし、追加resourceはresize / context loss / disposeで解放する。
